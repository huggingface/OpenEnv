...
"""Pi rollouts must only take ``reward.txt`` from verification, not from the agent."""
...
from pi_env.server.pi_environment import PiEnvironment, REWARD_FILE  # noqa: E402
...
def _rollout(monkeypatch, sandbox, verify, agent_commands=()) -> RolloutResult:
    ...
    monkeypatch.setattr(env, "_HFSandboxBackend", lambda **kwargs: None)
